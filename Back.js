...
async function acceso(username, contraseña, res) {
  ...
  BD.query(query, (err, result) => {
    ...
    for (let i = 0; i < result.length; i++) {
      if (
        result[i].Usuario == username &&
        result[i].Contraseña == contraseña &&
        result[i].TipoUsuario == "Administrador"
      ) {
...
app.post("/submit-cliente", uploadFields, (req, res) => {
  try {
    ...
    let query = `INSERT INTO Propiedad(ref_catastral,tipo_id,cedula,rif,asiento,n_protocolo,f_real,phone,sector,residentialcomplex,contactname,precio,tipo_oferta,tipo,niveles,tamano_terreno,tamanoconst,hab,bano,mediobano,servicio,maletero,terraza,oficina,pe,escaleras,piso,observaciones,referido,timestamp,propiedad,liberacion,catastral,solvencia,registro,poder,captacion) VALUES('${
      data.ref_catastral
    }','${data.tipo_id}','${data.cedula}','${data.rif}','${data.asiento}','${
      data.n_protocolo
    }','${data.f_real}','${data.phone}','${data.sector}','${
      data.residentialcomplex
    }','${data.contactname}','${data.precio}','${data.tipo_oferta}','${
      data.tipo
    }','${data.niveles}','${data.tamano_terreno}','${data.tamanoconst}','${
      data.hab
    }','${data.bano}','${data.mediobano}','${data.servicio}','${
      data.maletero
    }','${data.terraza}','${data.oficina}','${data.pe}','${data.escaleras}','${
      data.piso
    }','${data.observaciones}','${data.referido}','${
      data.timestamp
    }','${req.files.propiedad[0].path.replace(
      "\\",
      "\\\\"
    )}','${req.files.liberacion[0].path.replace(
      "\\",
      "\\\\"
    )}','${req.files.catastral[0].path.replace(
      "\\",
      "\\\\"
    )}','${req.files.solvencia[0].path.replace(
      "\\",
      "\\\\"
    )}','${req.files.registro[0].path.replace(
      "\\",
      "\\\\"
    )}','${req.files.poder[0].path.replace(
      "\\",
      "\\\\"
    )}','${req.files.captacion[0].path.replace("\\", "\\\\")}')`;
    //Carga de informacion de cliente e informacion general de inmueble
    BD.query(query, (err, result) => {
      console.log("Cargado con exito");
      BD.query(
        `SELECT idPropiedad FROM Propiedad WHERE ref_catastral= '${data.ref_catastral}'`,
        (err, result) => {
